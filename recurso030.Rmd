...
Aunque se trata de un método matemático de optimización (minimizar los errores), dado que los datos proceden de una muestra y que en el sistema de ecuaciones se incorpora el termino error ($\epsilon_{i}$) que posee caracteristicas estadísticas, se pueden realizar inferencias (intervalos de confianza y pruebas de hipótesis) al rededor de los estimadores encontrados y de los valores encontrados a partir del modelo estimado.
...
Está compuesto de una variable dependiente $y$, ($k$) variables independietes  y un componente aleatorio $\varepsilon$, formando un sistema de $n$ ecuaciones con $p = (k + 1)$ incógnitas correspondiendo al intercepto y los $k$ coeficientes de regresión, donde los $Y_i$ y las $X_{ij}$ toman valores conocidos en cada caso.
