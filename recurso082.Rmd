...
1. **Examinar la matriz de correlaciones entre las predictoras:** sea  $\text{COR}(\boldsymbol{X})$ una matriz cuyo elemento ($j,k$) corresponde a la correlación entre las predictoras $X_j$ y $X_k$, $\text{COR}\left(X_j, X_k\right)$.
...
2. **Factores de Inflación de Varianza:** denotados $\text{VIF}_j,\ j = 1, \ldots, k$ se calculan como: 
...
3. **Análisis de los valores propios de $\boldsymbol{X}'\boldsymbol{X}$:** se trata de evaluar si hay valores propios con valores cercanos a cero. Para ello se definen las medidas que se presentan a continuación:

**Número de condición:** mide la dispersión en el espectro de los valores propios de la matriz $\boldsymbol{X}'\boldsymbol{X}$. Se calcula como: $\kappa = \lambda_{\max} / \lambda_{\min}$. En $R$ se obtienen valores en raíz cuadrada, es decir, $\sqrt{\kappa}$, para el cual el criterio para detectar multicolinealidad es:
...
**Índice de condición:** es una medida útil para determinar el número de dependencias casi lineales en $\boldsymbol{X}'\boldsymbol{X}$. Se calcula como: $\kappa_j = \lambda_{\max} / \lambda_j,\ j = 1, \ldots, p$ (en $R$ se obtienen los valores $\sqrt{\kappa_j}$). El criterio para detectar multicolinealidad es:
...
**Proporciones de descomposición de varianza:** denotados $\pi_{ij}$ representan la proporción de la varianza de cada $\widehat\beta_j$ (o de cada factor de inflación de varianza) debida al $i$-ésimo valor propio de la matriz $\boldsymbol{X}'\boldsymbol{X}$.
